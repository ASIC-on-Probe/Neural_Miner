...
# Saturation limit
# ±250 µV corresponds to the nominal input range
CLIP_LIMIT = 250.0  # µV

# Minimum consecutive clipped samples
# to be regarded as a true saturation segment
MIN_CLIP_LENGTH = 5
...
print("=" * 70)
print("Reading neural recording data")
print("=" * 70)
...
print("\nSampling frequency")
print("-" * 50)
print(f"Specified Fs       : {FS:.2f} Hz")
print(f"Calculated from Time: {fs_from_time:.2f} Hz")

if abs(fs_from_time - FS) > 1:

    print(
        "WARNING: The sampling frequency calculated "
        "from the Time column differs from FS."
    )
...
# ============================================================
# 6. Detect continuous saturation segments
# ============================================================
...
for ch_idx, ch_name in enumerate(CHANNELS):
    ...
    clip_mask = np.abs(signal) >= CLIP_LIMIT
    ...
    for label_id in range(
        ...
        # Only regard sufficiently long continuous
        # saturation as a saturation segment
        if segment_length >= MIN_CLIP_LENGTH:
            ...
            start_time = time[start_sample]
            end_time = time[end_sample]
    ...
    all_clip_segments[ch_name] = segments
...
for ch_name, segments in all_clip_segments.items():
        ...
        clip_rows.append(
            {
                ...
                "Length (samples)": seg["length_samples"],
                "Start time (s)": seg["start_time_s"],
                "End time (s)": seg["end_time_s"],
...
# IMPORTANT:
#
# We do NOT physically delete saturated samples.
#
# We replace them with NaN so that the original time axis
# and sample positions are preserved.
...
for ch_idx, ch_name in enumerate(CHANNELS):
    ...
    for seg in all_clip_segments[ch_name]:
...
print("\nGenerating saturation detection plots...")
...
for ch_idx, ch_name in enumerate(CHANNELS):
    ...
    plt.title(
        f"{ch_name} - Saturation Detection"
    )
...
def calculate_psd_from_clean_segments(
    ...
    """
    Calculate Welch PSD using only continuous clean
    data segments.

    Saturated segments are represented by NaN and are
    excluded from PSD calculation.

    PSDs from individual clean segments are combined
    using the number of Welch windows as weights.
    """

    # Valid data
    valid_mask = np.isfinite(signal)

    # Find continuous valid regions
    labels, num_labels = ndimage.label(
        valid_mask
    )
...
# ============================================================
# 11. Calculate Welch PSD
# ============================================================

print("\n")
print("=" * 70)
print("Calculating Welch PSD")
print("=" * 70)
...
for ch_idx, ch_name in enumerate(CHANNELS):
    ...
    if f is None:
        ...
        print(
            f"WARNING: No sufficiently long "
            f"clean segment available for {ch_name}."
        )
...
if len(psd_results) > 0:
    ...
    # Only export 0–500 Hz
    frequency_mask = (
        (psd_df["Frequency_Hz"] >= FREQ_MIN)
        &
        (psd_df["Frequency_Hz"] <= FREQ_MAX)
    )
...
for ch_name in CHANNELS:
    ...
    f = psd_results[ch_name]["frequency"]

    pxx = psd_results[ch_name]["psd"]
...
plt.title(
    "Neural Signal Power Spectral Density"
)
...
print(
    "6. Welch_PSD_0_500Hz.xlsx"
)
print(
    "7. Welch_PSD_0_500Hz.png"
)
...
print(
    f"Saturation limit    = ±{CLIP_LIMIT:.1f} µV"
)
print(
